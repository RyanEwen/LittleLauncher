...
- **Release a new version:** Edit `<Version>` in `Directory.Build.props`, update fallback versions in `Package.wxs` and `Package.appxmanifest`, commit, tag `vX.Y.Z`, push. See `versioning.instructions.md` for the full checklist.
