...
**Grants were once seeded up front** (`SeedTrustedPermissionsAsync`), on the reasoning that a
well-built app *checks before it asks*: Teams reads `Notification.permission` on load, finds
`default`, shows "Stay in the know. Turn on desktop notifications." and renders its own in-page
banners, because as far as it can tell the desktop cannot show them. Seeding made the first read say
`granted`, so the loop was broken.

**That is gone, and the reasoning above is exactly what was wrong with it** - see below. Teams does
show its banner until the user answers the prompt once, and that is the correct trade: an answered
prompt is a permission the site set itself up for, and a seeded one is not.
...
Taking notifications out of the seeding was not enough, because the request handler still answered
Allow silently and the site still never ran its flow. The toggle is gone entirely, along with
`SeedTrustedPermissionsAsync`, `TrustedOrigins` and `ClearOnTrustDisabledAsync`. Every request is
answered once in the flyout's prompt bar and remembered per profile, exactly like a browser - which
is also the only behaviour that can be reasoned about, since `granted` now always means the site
asked and was answered.

The paragraphs above about `SavesInProfile` are kept because the finding still holds for the answers
the prompt bar records: `navigator.permissions.query()` and `Notification.permission` read the
stored setting and never see an unsaved grant, so an app that checks before asking believes it has
nothing.
...
- **Messenger notifies from a push handler**, so its page never constructs a `Notification` and
  ...
  ours, three times over, and all three are fixed: pre-granting notification permission meant it
  never ran the flow that registers its push worker; once it did register, the wrap was served to
  its own `importScripts` and the worker died in a stack overflow; and its script is served as the
  site's own HTML to anything but a request marked `Service-Worker: script`. **It is bridged now.**
  Kept here because it is the case every other launcher's problem was first mistaken for, and
  because it was called "not our bug" three times before it turned out to be three of ours.
...
response is served as the shim followed by the site's own script, **inlined**.

### The wrap can never import the URL it is served at

This cost most of a day, and four separate "fixes" that changed nothing. The wrap used to end in
`importScripts` of the original, at the same URL the wrap was served at. That **cannot** work: a
worker's `importScripts` resolves against the registration's script resource map, and the main
script is already in that map under its own URL, as the wrap. So the wrap imported itself, recursing
until `Uncaught RangeError: Maximum call stack size exceeded`, and the registration was torn down
leaving the origin with no worker at all.

**No request is ever made for it**, which is why nothing host-side could see or fix it: not a header
rule, not `Cache-Control: no-store`, not clearing the HTTP cache, not unregistering. The tell in a
stack trace is frames alternating between the script URL and `VM<n>`, the latter being imported
content evaluated as a script of its own:
...
It appeared to work only through **adoption**, which registers a *marked* URL: the wrap sat at
`sw.js?llbridge=1` and imported `sw.js`, two different URLs. So WhatsApp worked, and every site that
registered its worker plainly - Messenger, PrintStream - was broken by the bridge.

### Getting the original, which is the hard part

The original is fetched and inlined, so there is no import and no second URL. Preferably **by the
page**, whose `fetch` is the browser's own credentialed same-origin request; the app's own
`HttpClient` is the fallback.

**A site may serve its worker script only to a request carrying `Service-Worker: script`**, and
answer everything else with its app. Messenger does exactly this - a plain page fetch of
`/sw?s=push` returns 580KB of HTML, and the app's `HttpClient`, which *can* set the header but is
not the browser, gets an 841-byte interstitial. A page cannot set it either: it is a **forbidden
header name**, so `fetch()` drops it silently, and setting it through
`CoreWebView2WebResourceRequestedEventArgs.Request.Headers` does not reach the wire.

So the request is paused with the DevTools protocol and resumed with the header added
(`Fetch.enable` / `Fetch.continueRequest`), which is under no such restriction, being the channel
DevTools itself edits requests on. Messenger then returns 5.5KB of its real push worker. Three rules
hold that together:

- **Armed before the acknowledgement**, since the ack is what releases the page to fetch and the
  interception has to be running when the request goes out. Getting this backwards cost a build on
  an injection that never ran.
- **Scoped to the worker script URLs alone**, and it **always continues** the request, on every path
  out of the handler including the ones that failed to parse it. A paused request that is never
  resumed hangs, and this one is a script a registration is waiting on.
- **The page's fetch is bounded** (5s). `register()` waits on it, so an unbounded one holds the
  site's own registration open indefinitely, which reads as the launcher hanging on load.

**A body that does not look like a script is declined** and the site keeps its own worker: a
launcher with no bridge misses worker-scope notifications, while a launcher served half a wrap has
no working worker at all. A URL that fails is remembered for the session and the page is told not to
fetch it again, or an unbridgeable launcher re-downloads the same answer on every load.

Inlining also reaches sites adoption never could. **Teams** enforces Trusted Types and refuses to
let us create a policy, so its worker could not be adopted at all; inlining needs no URL to mint, so
it is now bridged. **Messages** likewise.
...
- **The wrap has to suit the realm it lands in.** Both realms get the original inlined, so what
  differs is only the *order*: a module's own `import` declarations are hoisted and its body expects
  to run first, so the shim goes after it, while a classic worker gets the patch in place before
  anything it does. The registration type still travels with the announcement, and a worker whose
  type is not known is not wrapped at all - the wrong order is survivable, but a wrap built for the
  wrong realm throws while the script is being evaluated, which fails the install and leaves the
  origin with **no worker at all**.

- **An already-registered worker does not pick up a changed wrap on its own.** Chromium re-fetches a
  worker's top-level script on its own schedule and installs a new one only if the bytes differ, so
  a launcher whose worker was installed under an older wrap keeps running it - a plain reload will
  not do it, and neither will `update()`. This matters once per wrap change: rare for a release,
  every build during development.

  The sweep detects it and **offers** a rebuild in the flyout's prompt bar, once per launcher per
  session. It pings the worker and the wrap answers with `WrapVersion`, so a worker running the
  current wrap is left alone. Two rules keep that prompt honest, both written after nearly shipping
  a wrong one:

  - **It is hedged where the evidence is.** A worker that does not answer is either unwrapped or
    wrapped by a build older than the reply itself, and those cannot be told apart. WhatsApp is the
    second and delivers notifications perfectly well, so "will not arrive" would have been a lie
    about a working launcher.
  - **It says what accepting costs**, because rebuilding resets the site's notification permission
    and drops its push subscription. A confident-sounding prompt could otherwise talk somebody into
    breaking a launcher that works.
