...
| `LittleLauncher.Pages` | All settings pages, LauncherBulkOps (item export/import, bookmark import) |
| `LittleLauncher.Services` | SftpSyncService, AutoSyncService, FaviconService, UpdateService, AppPickerService, AppCatalog, BrowserCatalog, BookmarkImport |
| `LittleLauncher.ViewModels` | UserSettings, AppPickerEntry |
| `LittleLauncher.Windows` | FlyoutWindow (+ `FlyoutWindow.EditMode.cs`), EditToolbarWindow, ItemEditorWindow, TextPromptWindow, LauncherSettingsWindow |
...
> **Note:** The original GitHub Copilot configuration (`.github/copilot-instructions.md`, `.github/instructions/`, `.github/prompts/`) is kept in place. If you change conventions here, mirror the change there (or vice versa) so the two formats don't drift.
