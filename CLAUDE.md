...
| Web page | `LauncherKinds.Web` (1) | `WebFlyoutWindow` on `Launcher.WebAddress` — the first of its `WebBookmarks` — with the rest as a bar along the bottom | Launcher settings — address (typed or chosen from browser bookmarks), bookmarks and size, with zoom/hidden-policy/reload/address-bar/pin/regular-window/browsing-data under **Advanced**; the bar itself edits bookmarks too |
