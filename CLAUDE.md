...
| Web page | `LauncherKinds.Web` (1) | `WebFlyoutWindow` on `Launcher.WebUrl`, or a bookmark bar when `WebUseBookmarks` is set | Launcher settings — address (typed or chosen from browser bookmarks) and size, with zoom/hidden-policy/reload/address-bar/pin/browsing-data under **Advanced** |
