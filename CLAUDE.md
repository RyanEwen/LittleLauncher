...
| `LittleLauncher.Models` | LauncherItem (its `ToString()` is the accessible name for every flyout row), Launcher, SshConnectionProfile |
...
| Web page | `LauncherKinds.Web` (1) | `WebFlyoutWindow` on `Launcher.WebUrl` | Launcher settings — address (typed or chosen from browser bookmarks) and size, with zoom/hidden-policy/reload/pin/browsing-data under **Advanced** |
